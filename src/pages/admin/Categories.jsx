          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan="4" className="py-12 text-center text-gray-400">No categories found.</td>
                </tr>
              ) : (
                categories.map((cat) => (
                  <tr key={cat.id} className="border-b border-cream-dark last:border-0 hover:bg-gray-50 transition-colors">
                    <td className="py-4 px-6 font-serif text-charcoal-dark font-medium">{cat.name}</td>
                    <td className="py-4 px-6 font-mono text-xs text-gray-500">{cat.slug}</td>
                    <td className="py-4 px-6 font-['Outfit'] text-sm">{cat.product_count || 0}</td>
                    <td className="py-4 px-6 text-right">
                      <div className="flex justify-end gap-3">
                        <button 
                          onClick={() => handleOpenModal(cat)}
                          className="text-gray-400 hover:text-ochre transition-colors"
                          aria-label="Edit"
                        >
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-cream/30 border border-cream-dark py-2.5 px-4 text-sm outline-none focus:border-turmeric transition-colors font-mono"
                    placeholder="e.g. whole-spices"
                  />
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full bg-cream/30 border border-cream-dark py-2.5 px-4 text-sm outline-none focus:border-turmeric transition-colors font-body resize-none"
